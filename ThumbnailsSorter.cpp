...
bool ThumbnailsSorter::lessThan(const QModelIndex &left, const QModelIndex &right) const{
	if(fullPathSorting)
		return QSortFilterProxyModel::lessThan(left, right);

	QString leftFile = sourceModel()->data(left, Qt::DisplayRole).toString().section('/', -1);
	QString rightFile = sourceModel()->data(right, Qt::DisplayRole).toString().section('/', -1);
